...
// Coin icon component based on tier
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/90 text-sm sm:text-base mt-0.5">
                  {promoOffers[currentPromo].coins} coins @ <span className="line-through opacity-70">₹{promoOffers[currentPromo].originalPrice}</span> ₹{promoOffers[currentPromo].price} <span className="ml-1">▶</span>
                </p>
              </div>
            ...
            <button
              ...
              className={`relative bg-[#1a1a1a] rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col items-center transition-all ${
                selectedPackage === pkg.id
                  ? "ring-2 ring-purple-500 bg-gradient-to-b from-[#2a1a3a] to-[#1a1a1a]"
                  : "hover:bg-[#222]"
              }`}
            >
              {/* Badge */}
              {pkg.badge && (
                <div className={`absolute -top-1 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium ${
                  pkg.badge === "Hot" 
        ...
        <div className="mt-6 text-center">
          ...
          <button className="text-purple-400 text-sm sm:text-base mt-1 underline underline-offset-2">
